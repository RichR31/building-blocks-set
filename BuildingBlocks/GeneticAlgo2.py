...
class GeneticAlgo:
    ...
    def crossover(self, population:list, total_offsprings:int):
        """
        ...
        """
        crossed_offsprings = []
        type1 = total_offsprings//3
        type2 = total_offsprings - type1
    ...
    def mutate(self, puplulation, total_offsprings):
        """
        makes the given number of mutated offsprings from a list of elite individuals
        ...
        for i in range(total_offsprings):
            offspring = self.mutate_child_from(puplulation[i%len(puplulation)])
    ...
    def run(self, additional_individuals:list=None):
        ...
        for generation in range(0,self.generations):
            ...
            new_individuals['c'] = self.crossover(population_list, self.crossed_size)
            
            # Apply mutation to create new offspring off of the elite individuals
            new_individuals['m'] = self.mutate(population_list, self.mutated_size)
